...
- `candidate_ids` are opaque: the `document.search` capability creates a frozen `DocumentDiscoveryCandidate(candidate_id, document_id, document_revision)` owned by its originating checkpointed `AgentResult.data`; the ephemeral `DiscoveryCandidateRegistry` (`tools/discovery_candidates.py`) indexes those back as `candidate_id -> DocumentDiscoveryCandidate` and is **not** a separate persistent store (no table/model/migration). The model observation never exposes `document_revision`, and the **Binding Resolver is the sole owner that creates/pins the binding**, revalidating current ACL/scope and using the candidate's immutable discovered revision.
