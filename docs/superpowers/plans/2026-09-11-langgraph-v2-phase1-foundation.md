...
def normalize_object_key(raw_key: str) -> str:
    """Storage-reported key, used verbatim: no URL-decoding, no case folding, no
    filename reconstruction; strip exactly one leading '/'; reject empty/absolute/'..'."""
...
def _version_token(version_id: str | None, etag: str, size_bytes: int) -> str:
    # versionId wins when storage versioning is on; otherwise etag+size is the surrogate.
    return f"version:{version_id}" if version_id else f"etag:{etag}:size:{size_bytes}"

def compute_source_object_identity(
    bucket: str, object_key: str, version_id: str | None, etag: str, size_bytes: int, content_sha256: str
) -> str:
    """Immutable identity of one stored object version. content_sha256 is streamed from
    storage by the authoritative trigger; a multipart etag ('<md5>-<n>') is NOT a hash."""
    key = normalize_object_key(object_key)
    return f"{SOURCE_SCHEME}|{bucket}|{key}|{_version_token(version_id, etag, size_bytes)}|{size_bytes}|{content_sha256}"

def arrival_identity(bucket: str, object_key: str, version_id: str | None, etag: str, size_bytes: int) -> str:
    """Webhook-only staging key: lets /confirm match an arrival before bytes are hashed."""
    key = normalize_object_key(object_key)
    return f"{SOURCE_SCHEME}|{bucket}|{key}|{_version_token(version_id, etag, size_bytes)}|{size_bytes}"
...
Canonicalization rules, applied identically by every trigger: `bucket` as reported by storage; `object_key` verbatim from the storage event/list API (no URL-decoding of `+`/`%XX`, no case folding, no reconstruction from the uploaded filename); exactly one leading `/` stripped and empty/absolute/`..` keys rejected; `version_id` preferred over `etag+size` when MinIO/S3 versioning is enabled; `content_sha256` is the streamed SHA-256 computed by `/confirm`, direct upload, chat upload, or reindex (never trusted from a client header); a multipart etag (`<md5>-<n>`) is never treated as content identity, so `sha256` is always part of the string. The identity is immutable for one stored object version: overwriting the same key with different bytes changes version/etag/sha → new identity → new attempt; identical bytes with versioning off converge to the same identity (idempotent). The `ObjectCreated` webhook writes only `source_arrivals.arrival_identity`; `/confirm` matches that arrival, hashes the object, and calls `get_or_create_ingestion_attempt(document_id, compute_source_object_identity(...), resolve_build_profile(...))`.
...
**Interfaces:**
- Produces: two independent batchers — `run_evidence_payload_gc_batch` (evidence retention only) and `run_revision_artifact_gc_batch` (revision artifacts only) — plus concrete object/vector artifact delete methods and a scheduled one-shot worker that drives both.
