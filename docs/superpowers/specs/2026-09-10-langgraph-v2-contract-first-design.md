...
| Logical read requirement | complex `TargetUnit`; fast typed AgentRequest input |
| Invocation `task_id` | complex TaskPlan or fast dispatcher |
...
├── direct
├── clarify
├── fast domain → AgentRequest → shared capability → AgentResult
└── complex research → TaskPlan → AgentRequest → shared capability → AgentResult
        ↓
...
`ScopedDocument` owns document ID, pinned immutable revision, and role only. `DocumentReference.revision_requirement` is the authoritative semantic instruction for pinned versus current/latest resolution. `BindingRevisionRequirement` is the minimal hot-path relation for a binding created from a semantic reference; it copies no policy value. Such a binding has exactly one relation, both IDs must resolve, and conflicting references must be clarified before binding. A discovered binding has no semantic revision instruction and therefore no relation: its candidate revision is an explicit immutable pin. Resume and evidence-reuse validation follow the relation when present; otherwise they validate the exact pinned revision. Promotion preserves that exact pin unless the user supplies a new semantic reference requiring current/latest resolution, which creates a newly resolved binding. Audit provenance is not required for this decision. Downstream reads still consume only the pinned `ScopedDocument`. Target/reference roles are required; supporting/discovered roles are optional. There is no redundant `required` boolean.
...
A discovery candidate is consumed inside its originating `AgentResult`, whose `task_id` already supplies task lineage. Reasons and minimal trigger lineage are written once by the Binding Resolver to `BindingProvenance`; evidence IDs are not copied because `EvidenceUse → task_id` reconstructs evidence/task lineage.

Revision is pinned when a binding is created. Binding Resolver MUST resolve an immutable revision identity whose `ContentLocator` coordinate system remains stable for that revision. Downstream reads never silently switch to latest. On resume or reuse, `CurrentRevisionRequirement` is reevaluated against the authoritative revision record and causes explicit rebinding/reacquisition when required; `PinnedRevisionRequirement` must still match exactly. Neither requirement is copied onto `ScopedDocument`.
...
A `fast_domain` route dispatches the already-selected shared capability directly; it does not construct a TaskPlan or planned DAG. The fast dispatcher allocates a deterministic invocation `task_id` and, for bounded document/read operations, a `target_id` whose binding/requested-locator definition is carried by the typed AgentRequest input. Fast bounded reads use the operation's fixed typed success/coverage semantics rather than a generic CompletionCriterion collection. The persisted AgentRequest is authoritative for that fast invocation. For complex execution, TaskPlan owns planned task IDs and TargetUnits. AgentResult and EvidenceUse refer to the invocation `task_id` in both paths, without implying that every invocation came from a TaskPlan.

A `direct` conversational response such as a greeting executes no capability and keeps `ExecutionState.plan=None`; a direct factual response is not permitted. Synthesis-only reuse does not create a synthetic plan or `evidence.reuse` TaskSpec. Existing current-run uses are revalidated and admitted directly. If cross-run reuse must create a new use, the fast dispatcher owns an explicit reuse-validation invocation ID and calls the validator directly; the resulting EvidenceUse points to that invocation, not to a fake planned DAG task.
...
Completion criteria belong only to logical `TargetUnit`s; the containing TargetUnit is the authoritative target, so CoverageCriterion does not repeat `target_id`. A target may contain at most one CoverageCriterion, and SemanticCriterion IDs must be unique within that target; plan validation rejects violations. Consequently `MissingRequirement(target_id, criterion_kind="coverage")` identifies at most one criterion, while a semantic missing requirement carries an ID that resolves uniquely in the same target. Coverage criteria are deterministic. `SemanticCriterion` is reserved for a concrete target-level judgment such as legal compliance, is created only by trusted planner policy, and goes to the semantic evaluator. Partial coverage requires explicit user/objective semantics and a deterministic reason.
...
`TaskPlan.goal` is the user-level research goal; `TaskSpec.task_objective` is the operation-specific intent consumed by one capability. Task inputs reference `target_id`/`binding_id` when needed; they do not copy `ScopedDocument`. Runtime resolves IDs from authoritative plan/binding set. Child semantic scope is a subset of parent bindings except policy-authorized reference discovery. Task execution success uses typed capability semantics; user-requirement criteria remain only on TargetUnit.
...
- every use belongs to the current `EvidenceUseEnvelope.run_id`, resolves `task_id` to one explicit invocation owned by the fast dispatcher or by the complex TaskPlan, and resolves `evidence_id` to one current-authorized EvidenceRecord;
- `purpose="coverage"` requires a non-null `target_id` resolving to the invocation's authoritative target definition: typed AgentRequest input for fast execution or TargetUnit for complex execution. Document role/revision/locator then derive through its binding and ScopedDocument;
- `purpose="discovery"` requires `target_id=None` and is ineligible for synthesis or claim support;
- a `supporting` use with a target follows the same target/binding path; a targetless supporting use is synthesis-eligible only for a validated targetless invocation, with relevance derived from the fast AgentRequest objective/input or complex TaskSpec task_objective/input and source kind derived from its EvidenceRecord. Its document role is `None` because no canonical binding path exists.
...
`DocumentBindingSet.unresolved_ref_ids` and `ClarificationRequest.unresolved_ref_ids` reference authoritative `SemanticContext.document_refs`; neither copies DocumentReference objects. Candidate identity/order is stable within a clarification. `selected_candidate_id` is the authoritative deterministic selection; the raw clarification reply remains authoritative in ChatMessage persistence and is not copied here. `ClarificationRequest.expires_at` is retained because resume consumes a stable per-request deadline that must not change after deployment TTL configuration changes. Resume validates expiry, candidate membership, and current ACL, then asks Binding Resolver to create a binding. It does not rerun unconstrained ambiguity resolution.
...
5. Semantic DocumentReference owns pinned/current RevisionRequirement; Binding Resolver alone owns resolved document role and pinned revision.
...
11. Complex TargetUnit owns locator/criteria; fast typed AgentRequest input owns its bounded target definition; binding owns document identity/revision/role.
...
33. EvidenceUse task IDs resolve to explicit fast-dispatch or complex-plan invocations; only complex execution requires TaskPlan.
...
| current `user_id`, run lineage, ACL/deadline/config | backend runtime | authenticated ingress/runtime | capability and evidence services | no business state | refreshed | REMOVE from RequestContext; runtime only |
...
| per-reference pinned/current `RevisionRequirement` | Semantic Finalizer | semantic/reference resolver | Binding Resolver, resume/reuse validator | snapshot | no | KEEP: authoritative freshness instruction; not copied to binding |
...
| complex `target_id → binding/locator/criteria` | TaskPlan | planner | capability, evaluator, hydrator | checkpoint | no | KEEP: logical complex requirement |
| fast target definition | persisted typed AgentRequest input | fast dispatcher | capability, evaluator, hydrator | request boundary | no | KEEP: bounded fast requirement without a TaskPlan |
| `plan_id/goal/tasks` | TaskPlan | planner | validator, scheduler, evaluator | checkpoint for complex path | no | KEEP: executable complex plan; goal differs from task objective |
| invocation `task_id` | fast dispatcher or TaskPlan | owning orchestrator | capability, fan-in, EvidenceUse resolver | request/result/use boundaries | no | KEEP: common invocation identity without implying a DAG |
| complex capability/objective/input/dependencies/origin | TaskPlan | planner | scheduler; selected capability consumes objective/input | checkpoint | no | KEEP: complex operation and DAG definition |
...
| `SynthesisInput.semantic/evaluation/use refs` | semantic/evaluator/use store | orchestration | synthesizer/hydrator | no separate persistence | no | KEEP minimal factual request; evaluation is mandatory and sufficient; plan or fast request plus bindings/runtime go only to hydrator |
...
| `AnswerClaim.claim_id/text/evidence_ids` | Answer layer | synthesizer | Grounding | internal draft | text overlaps answer | KEEP: standalone grounding unit without offsets; one mapping only |
...
| `ScopedDocument.required/locator/revision_policy/lineage` | required derived from role; locator in target definition; RevisionRequirement stays with semantic DocumentReference; lineage in BindingProvenance |
...
| TaskSpec copied ScopedDocuments + completion criteria + three replan fields | ID-bearing typed input + `TaskOrigin`; complex criteria remain on TargetUnit, fast target semantics stay in typed AgentRequest input |
...
| candidate ranking/debug/source-task copies | resolver telemetry/container AgentResult; clarification keeps identity/order/label only |
...
- factual fast routes dispatch shared capabilities directly without TaskPlan, while complex routes alone construct TaskPlan;
...
- pinned/current RevisionRequirement survives semantic persistence, drives binding/resume/reuse validation, and is not copied into ScopedDocument;
...
- RequestContext alone owns original_query; SemanticDraft and SemanticContext do not copy it, SemanticContext carries no binding-ID projection, and binding/clarification unresolved sets reference semantic documents only by ref_id;
...
| 3 | Applied with retained semantic instruction | Resolved binding contains pinned revision only; typed RevisionRequirement remains on authoritative DocumentReference for Binding Resolver and resume/reuse validation. |
...
This revision is **Approved design**. Independent acceptance review confirmed the final topology, ownership, criterion, synthesis, and checkpoint corrections; the last discovered-binding revision gap was closed by distinguishing reference-governed freshness from exact candidate pins. Implementation planning may now begin. Do not reopen canonical fields without a demonstrated current producer, consumer, and non-derivable invariant.
