...
try:
    ...
    if hasattr(settings, "model_snapshot"):
        ...
        model_snap = {"provider": getattr(settings, "NEXUSRAG_LLM_PROVIDER", "unknown"), "model": getattr(settings, "NEXUSRAG_LLM_MODEL", "unknown")}
...
try:
    corpus_rev = "unknown"
    if Path("/tmp/corpus_revision").exists():
        corpus_rev = Path("/tmp/corpus_revision").read_text().strip()
except Exception:
    corpus_rev = "unknown"
...
md = {
    ...
    'flags': {
        'NEXUSRAG_SEMANTIC_PREPROCESSOR': getattr(settings, 'NEXUSRAG_SEMANTIC_PREPROCESSOR', False) if 'settings' in dir() else 'unknown',
        'NEXUSRAG_COMPLEXITY_SHADOW': getattr(settings, 'NEXUSRAG_COMPLEXITY_SHADOW', False) if 'settings' in dir() else 'unknown',
        'NEXUSRAG_COMPLEXITY_ACTIVE': getattr(settings, 'NEXUSRAG_COMPLEXITY_ACTIVE', False) if 'settings' in dir() else 'unknown',
        'NEXUSRAG_DEEP_ENABLED': getattr(settings, 'NEXUSRAG_DEEP_ENABLED', False) if 'settings' in dir() else 'unknown',
        'NEXUSRAG_DEEP_SHADOW': getattr(settings, 'NEXUSRAG_DEEP_SHADOW', False) if 'settings' in dir() else 'unknown',
        'NEXUSRAG_AGENT_DEADLINE_SECONDS': getattr(settings, 'NEXUSRAG_AGENT_DEADLINE_SECONDS', 28) if 'settings' in dir() else 'unknown',
        'NEXUSRAG_DEEP_MAX_PARALLEL': getattr(settings, 'NEXUSRAG_DEEP_MAX_PARALLEL', 2) if 'settings' in dir() else 'unknown',
        'NEXUSRAG_DEEP_MAX_DOMAIN_CALLS': getattr(settings, 'NEXUSRAG_DEEP_MAX_DOMAIN_CALLS', 6) if 'settings' in dir() else 'unknown',
    },
...
baseline = {
    "meta": {
        "commit": "${PRE_COMMIT}",
        "label": "${LABEL_PRE}",
        "captured_at": "pre_task1_capture_time",
    },
    "aggregate": {
        "latency_p50": 0.0,
        "latency_p95": 0.0,
...
baseline = {
    "meta": {
        "commit": "${POST_COMMIT}",
        "label": "${LABEL_POST}",
        "captured_at": "post_task1_capture_time",
    },
    "aggregate": {
        "latency_p50": 0.0,
        "latency_p95": 0.0,
