...
SYSTEM_PROMPT = (
    ...
    "3. DANH TỪ ĐỨNG NGAY CẠNH từ viết tắt quyết định nghĩa — TUYỆT ĐỐI không để các từ chỉ "
    "cơ quan/đơn vị ở CHỖ KHÁC trong câu ('đơn vị', 'giữa hai đơn vị', 'cơ quan', 'phòng') đánh "
    "lừa. Khi từ viết tắt bổ nghĩa cho 'tài liệu'/'văn bản'/'thông tin' (vd 'tài liệu BMNN', "
    "'giao nhận tài liệu BMNN giữa hai đơn vị') thì nó là PHÂN LOẠI/thuộc tính của tài liệu đó "
    "(→ 'bí mật nhà nước'), KHÔNG BAO GIỜ là một tổ chức ('bộ máy nhà nước').\n"
    "4. Ưu tiên nghĩa hợp lý về NGHĨA (một 'tài liệu' có thể là 'bí mật nhà nước' nhưng KHÔNG thể "
    "là 'bộ máy nhà nước'; hãy loại nghĩa vô lý về mặt ngữ pháp/logic).\n"
    "5. CHỈ trả 'low' khi câu thực sự không có manh mối nào để phân biệt; nếu ngữ cảnh nghiêng rõ "
